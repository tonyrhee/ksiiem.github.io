...
- 1. START WITH, 2. CONNECT BY, 3. WHERE 조건은 **계층 시작 조건, 계층 전개 조건, 필터 조건**으로 세 가지 조건은 별개이다.(처리 순서대로 번호 부여)
...
Q. WINDOW FUNCTION을 사용하지 않고 아래의 실행 결과를 출력하는 SOL로 가장 적절한 것은?
...
- 메인쿼리와 서브쿼리
    ...
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/f9203d7d-db80-44c9-9dd1-c3027fd6404e/Untitled.png)
...
- DENSE_RANK는 순위를 구하는 함수로 동일한 순위를 하나의 등수로 취급한다.
...
- RATIO_TO_REPORT는 파티션 내에 주어진 칼럼 값의 합계에 대한 행별 백분율을 소수점으로 구하는 함수이다.
    ...
    - PARTITION BY GRP로 GRP 컬럼의 값으로 두 그룹으로 나누고, VAL1 값의 합에 대한 비율을 구한다.
- NTILE는 파티션별 전체 건수를 주어진 인자로 N 등분한 결과를 반환한다.
    ...
    ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/10cb8912-f9f7-465b-a584-3dbdc7aef40d/478f38f8-eb7f-468b-9e8b-af5d5da180c7.png)
    
    ![스크린샷 2024-05-17 오후 6.11.57.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/1c7e0174-a0c8-4910-83cc-fbdd37785947/7ee5dac4-3538-410a-99ce-e2d6f2ad2a27.png)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/44aa6fc4-1b76-4da5-b4c8-b34c2da1d43b/Untitled.png)
...
- 서브쿼리를 사용할 때 주의사항
    - 서브쿼리를 괄호로 감싸서 사용한다.
    - 서브쿼리는 단일행(Single Row) 또는 복수행(Multiple Row) 비교 연산자와 함께 사용 가능하다. 단일 행 비교 연산자는 서비쿼리의 결과가 반드시 1건 이하이어야 하고 복수 행 비교 연산자는 서비쿼리의 결과 건수와 상관없다.
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/faaa6c28-302e-4da5-8ad2-0b158fa41a95/Untitled.png)
...
① 이벤트 시작일자가 ‘2014.10.01'과 같거나 큰 이벤트를 대상으로 이 메일이 발송된 기록이 있는 모든 회원을 추출하는 SQL이다.
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/de3e22dd-e8a0-4577-9313-d0605472237f/Untitled.png)

```sql
SELECT B.상품ID, B.상품명, C.평가항목ID, C.평가항목명, A. 평가
회차, A. 평가등급, A.평가일자
FROM
평가결과 A, 평가대상상품 B, 품질평가항목 C, (SELECT MAX(평가회차) AS 평가회차 FROM 평가결과) D
WHERE A.상품ID = B.상품ID
AND
A.평가항목ID = C. 평가항목ID
AND
A.평가회차 = D.평가회차;
② SBLECT B.상품ID, B.상품명, C.평가항목ID, C.평가항목명, A.평가
회차, A.평가등급, A.평가일자
FROM
...
FROM 평가결과 X WHERE X.상품ID = B.상품ID AND X.평가항목ID = C.평가항목ID);
③ SELECT B.상품ID, B.상품명, C.평가항목ID, C.평가항목명 , MAX(A.평가회차) AS 평가회차
,MAX(A. 평가등급) AS 평가등급 MAX(A. 평가일자) AS 평가일자
FROM
평가결과 A, 평가대상상품 B, 품질평가항목 C
WHERE A.상품ID = B.상품ID AND A.평가항목ID =C.평가항목ID CROUP BY B.상품ID, B.상품명, C.평가항목ID, C.평가항복명:
...
- 인라인 뷰(Inline View)
    ...
    - 인라인 뷰는 SQL문이 실행될 때만 암시적으로 생성되는 동적인 뷰이기 때문에 데이터베이스에 해당 정보가 저장되지 않는다.
...
- COL3 구문은 전체 데이터에서 현재 행부터 현재 행 까지의 범위에서 가장 큰 연봉을 구한다.
    ...
    - 연봉 내림차순하여 현재 행부터 현재 행 까지 높은 연봉을 출력하게 되면 각 행마다 본인 연봉이 나오게 된다. 전체에서 최대 연봉 값이지만 현재 행은 1행 뿐이므로 MAX 값은 의미가 없으며 자기 자신의 값이 출력된다.
- WHERE COL1 = 2
    - COL1의 결과가 2인 행만 본다.(⭐️)
- COL별로 정렬 기준이 다르며, 독립적인 것만 주의하면 된다.
    
    ![1715950024.402916.JPEG](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/27ea6a01-6e1b-4fdf-b908-925ff9394b6c/1715950024.402916.jpeg)
...
![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/3e2d8e2b-4ab3-463b-b14f-cbdd834a0993/cac32789-4847-438f-9fe9-152a839550be/Untitled.png)
...
- GROUPING 함수는 이들 그룹을 구별하는 함수이다.
    ...
    - 따라서 Grouping(지역ID) = 1 일때는 지역전체, 아닌 경우에는 지역명이 되는 것이다.
    이용일시도 마찬가지이다.
...
- 서브쿼리는 단일 행 또는 복수 행 비교 연산자와 함께 사용 가능하다. 단일 행 비교 연산자는 서비쿼리의 결과가 반드시 1건 이하여야 하고, 복수 행 비교 연산자는 서브쿼리의 결과 건수와 상관없다.
